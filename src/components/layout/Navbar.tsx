...
const Navbar: React.FC = () => {
      ...
      <List className="!pt-8 !px-4 flex flex-col items-center">
        {menuItems.map((item) => (
          <ListItem
            ...
            onClick={handleDrawerToggle}
            sx={{width:'90%'}}
            className={`mb-2 rounded-lg transition-all duration-300 hover:bg-edu-dark hover:text-white cursor-pointer ${pathname === item.path ? 'bg-edu-dark text-white' : 'text-edu-dark bg-white bg-opacity-100'
              }`}
          >
            <ListItemText
              primary={item.text}
  ...
  return (
    <>
      ...
      >
        <Toolbar className="px-4 md:px-8 flex justify-between">
              ...
              <Box className="flex gap-6 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                {menuItems.map((item) => (
                  <Link
                    key={item.text}
                    href={item.path}
                    className={`px-4 py-2 rounded-lg transition-all duration-300 no-underline font-medium ${pathname === item.path
